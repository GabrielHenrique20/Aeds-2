/*Repita a questão de Ordenação por Seleção, contudo, usando o algoritmo Counting Sort, fazendo com que a chave de pesquisa seja o atributo yearOfBirth. 
...
public class TP02Q11 {
    ...
    // Metodo para preencher um vetor
    ...
    public static Personagem[] coutingSort(Personagem[] personagens) {
        ...
        for (int i = 0; i < personagens.length; count[personagens[i]
                .getYearOfBith()]++, i++, movimentacoes++, comparacoes++)
            ;

        for (int i = 1; i < count.length; count[i] += count[i - 1], i++, movimentacoes++, comparacoes++)
            ;

        for (int i = personagens.length - 1; i >= 0; ordenado[count[personagens[i].getYearOfBith()]
                - 1] = personagens[i], count[personagens[i].getYearOfBith()]--, i--, movimentacoes++, comparacoes++)
            ;
    ...
    public static void log(long tempoExecucao, int comparacoes, int movimentacoes) {
        File log = new File("775799_countingsort.txt");
