...
class Agenda {
    ...
    No inserir(Contato contato, No i) {
        if (i == null) {
            i = new No(contato.nome.charAt(0)); // Criando um novo nó com base no primeiro caractere do nome do contato
        }
        ...
        if (contato.nome.charAt(0) == i.letra) {
            ...
            i.esq = inserir(contato, i.esq); // Inserir no ramo esquerdo
        } else {
            i.dir = inserir(contato, i.dir); // Inserir no ramo direito
        }
        return i;
    }
	...
	private boolean pesquisarNome(No no, String nome) {
        ...
        if (no == null) {
            resp = false;
        } else if (Character.toUpperCase(nome.charAt(0)) == no.letra) { // Corrigindo o método de conversão de caractere
            resp = false;
            for (Celula i = no.primeiro.prox; (!resp && i != null); i = i.prox) {
                if (i.contato.nome.equals(nome)) {
                    resp = true;
                }
            }
        } else if (Character.toUpperCase(nome.charAt(0)) < no.letra) { // Corrigindo o método de conversão de caractere
